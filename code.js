...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.editarPlano = function(index) {
        ...
        if (plano.dataFim) { // Assume 'por datas' if dataFim exists (could be more robust)
            definirPorDatasRadio.checked = true;
            definirPorDiasRadio.checked = false;
            periodoPorDatasDiv.style.display = 'block';
            periodoPorDiasDiv.style.display = 'none';
            ...
            dataInicioInputDias.value = ''; // Clear other date inputs
            numeroDiasInput.value = '';
        } else if (plano.diasPlano && plano.diasPlano.length > 0) { // Fallback to 'por dias' if diasPlano exists and no dataFim (less reliable, improve logic if needed)
            ...
            numeroDiasInput.value = plano.diasPlano.length; // Approximate days based on plan days
            dataInicioInputDatas.value = ''; // Clear other date inputs
        ...
        if (plano.periodicidade === 'dias-semana') {
            ...
            if (plano.diasSemana) { // Check if diasSemana property exists
                plano.diasSemana.forEach(dia => {
                    const checkbox = document.getElementById(dia); // Assuming ids are like "segunda", "terca", etc.
        ...
        // Change form submission to update existing plan
        formPlano.onsubmit = function(event) {
            event.preventDefault();
            atualizarPlano(index); // Call a new function to update
        };

        // Optionally, change the submit button text to "Atualizar Plano"
        formPlano.querySelector('button[type="submit"]').textContent = 'Atualizar Plano';
    };

    function atualizarPlano(index) {
        const tituloLivro = document.getElementById('titulo-livro').value;
        const totalPaginas = parseInt(document.getElementById('total-paginas').value);
        let dataInicio, dataFim;
        ...
        const planoAtualizado = criarPlanoLeitura(tituloLivro, totalPaginas, dataInicio, dataFim, periodicidade, diasSelecionados);
        if (planoAtualizado) {
            planos[index] = planoAtualizado; // Update the plan at the specific index
            salvarPlanos(planos);
            renderizarPlanos();
            formPlano.reset();
            formPlano.onsubmit = function(event){ // Reset form submission to create new plan
                event.preventDefault();
                formPlano.dispatchEvent(new Event('submit')); // Re-trigger original submit event
            };
            formPlano.querySelector('button[type="submit"]').textContent = 'Criar Plano'; // Reset button text
            diasSemanaSelecao.style.display = 'none';
            periodoPorDatasDiv.style.display = 'block'; // Reseta para definição por datas
            periodoPorDiasDiv.style.display = 'none';
            definirPorDatasRadio.checked = true;
            definirPorDiasRadio.checked = false;
            alert("Plano de leitura atualizado com sucesso!");
        }
    }
    ...
    window.recalcularPlano = function(index) {
        ...
        // If there are past unread days, you might want to redistribute those pages too, or just acknowledge the delay.
        // For now, we are only adjusting future pages.
