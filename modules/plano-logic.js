...
export function analisarCargaSemanal(planos, totalPlanos) {
    ...
    planos.forEach((plano, index) => {
        // Ignora planos concluídos ou inválidos para a análise de carga futura
        const status = determinarStatusPlano(plano);
        if (status === 'concluido' || status === 'invalido') {
            return;
        }

        // MELHORIA: Calcula a média com base nos dias e páginas restantes
        const diasDeLeituraRestantes = plano.diasPlano?.filter(d => d.data && !d.lido).length || 0;
        if (diasDeLeituraRestantes === 0) {
            return; // Evita divisão por zero se não houver dias de leitura para frente
        }
        ...
        const diasSemanaPlano = plano.diasSemana || []; // array de números [0-6]
        const numeroDoPlano = totalPlanos - index; // Numeração visual (ex: #5, #4...)
        
        if (plano.periodicidade === 'diario') {
            diasDaSemana.forEach(dia => {
                dia.planos.push({ numero: numeroDoPlano, media: mediaPaginasDia });
