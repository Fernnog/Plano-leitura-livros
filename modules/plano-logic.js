 ...
 * @returns {string} Uma string formatada como um arquivo iCalendar.
 */
export function gerarConteudoICS(planos, horaInicio, horaFim) {
    ...
    let icsContent = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//SeuApp//Gerenciador de Leitura//PT',
    ];
    ...
    planos.forEach(plano => {
        plano.diasPlano.forEach(dia => {
            ...
            dataFimEvento.setUTCHours(endHour, endMinute, 0, 0);

            icsContent.push(
                'BEGIN:VEVENT',
                `UID:${plano.id}-${dia.data.getTime()}@seuapp.com`,
                `DTSTAMP:${formatarDataICS(new Date())}`,
                `DTSTART;TZID=Etc/UTC:${formatarDataICS(dataInicioEvento)}`,
                `DTEND;TZID=Etc/UTC:${formatarDataICS(dataFimEvento)}`,
