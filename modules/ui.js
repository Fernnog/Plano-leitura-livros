...
/**
 * Cria e dispara o download de um arquivo .ics.
 * @param {string} conteudoICS - O conteúdo do arquivo a ser baixado.
 * @param {string} nomeArquivo - O nome sugerido para o arquivo.
 */
export function dispararDownloadICS(conteudoICS, nomeArquivo) {
    const blob = new Blob([conteudoICS], { type: 'text/calendar;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `${nomeArquivo}.ics`);
...
/** Mostra o modal de exportação de agenda. */
export function showExportModal(planos) {
    renderExportModalContent(planos);
    DOMElements.exportModal.classList.add('visivel');
}

/** Esconde o modal de exportação de agenda. */
export function hideExportModal() {
    DOMElements.exportModal.classList.remove('visivel');
}

/** Renderiza a lista de planos ativos dentro do modal de exportação. */
function renderExportModalContent(planos) {
    ...
    if (planosAtivos.length === 0) {
        DOMElements.exportPlanosListContainer.innerHTML = '<p>Não há planos em andamento para exportar.</p>';
        return;
    }

    const html = planosAtivos.map((plano) => `
        <div class="export-plano-item">
            <input type="checkbox" id="export-plano-${plano.id}" data-plano-index="${planos.findIndex(p => p.id === plano.id)}">
            <label for="export-plano-${plano.id}">${plano.titulo}</label>
        </div>
    `).join('');

    DOMElements.exportPlanosListContainer.innerHTML = html;
}
