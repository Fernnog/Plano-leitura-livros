...
export function showCadastroForm(planoParaEditar = null) {
    ...
    DOMElements.planosPausadosSection.style.display = 'none'; // Esconde o novo painel também
    ...
    planos.forEach((plano, index) => {
        ...
        const diasLeituraHTML = plano.diasPlano.map((dia, diaIndex) => `
            <div class="dia-leitura ${dia.lido ? 'lido' : ''}">
                <input type="checkbox" id="dia-${index}-${diaIndex}" data-action="marcar-lido" data-plano-index="${index}" data-dia-index="${diaIndex}" ${dia.lido ? 'checked' : ''} ${isPausado ? 'disabled' : ''}>
        ...
        const podeRecalcular = (status === 'atrasado' || status === 'em_dia') && !isPausado;
        const avisoHTML = podeRecalcular ? `
            ...
            <div class="plano-leitura card-${status}" id="plano-${index}">
                <div class="plano-header">
                    ...
                    <div class="plano-acoes-principais">
                        ${!isPausado ? `<button data-action="editar" data-plano-index="${index}" title="Editar Plano"><span class="material-symbols-outlined">edit</span></button>` : ''}
                ...
                <details class="dias-leitura-details" ${isPausado ? 'style="pointer-events: none; opacity: 0.7;"' : ''}>
...
function renderizarPainelProximasLeituras(planos, totalPlanos) {
    ...
    const todasAsProximasLeituras = planosAtivos.flatMap((plano, planoIndex) =>
        plano.diasPlano
            .filter(dia => dia.data && new Date(dia.data) >= hoje && !dia.lido)
            .map(dia => ({ ...dia, titulo: plano.titulo, planoIndex: planos.findIndex(p => p.id === plano.id) }))
...
function renderizarPainelLeiturasAtrasadas(planos, totalPlanos) {
    ...
    const todasAsLeiturasAtrasadas = planosAtivos.flatMap((plano, planoIndex) =>
        plano.diasPlano
            .filter(dia => dia.data && new Date(dia.data) < hoje && !dia.lido)
            .map(dia => ({ ...dia, titulo: plano.titulo, planoIndex: planos.findIndex(p => p.id === plano.id) }))
    ).sort((a, b) => a.data - b.data);
...
function renderizarQuadroReavaliacao(dadosCarga) {
    ...
    dadosCarga.forEach(dia => {
        ...
        html += `<tr><td>${dia.nome}</td><td class="planos-dia-cell">${planosDoDia}</td><td class="total-paginas-dia">${dia.totalPaginas}</td></tr>`;
    });
...
function renderizarLegendaReavaliacao(planos, totalPlanos) {
    ...
    planos.forEach((plano, index) => { mapaPlanoNumero[index] = planos.length - index; });
    planosAtivos.forEach((plano) => {
        ...
        legendaHTML += `<li class="reavaliacao-legenda-item"><span class="numero-plano-tag">${numeroPlano}</span><span>${plano.titulo}</span></li>`;
    });
    legendaHTML += '</ul>';
