...
// NOVA FUNÇÃO: Renderiza o conteúdo da tabela de reavaliação.
/**
 * Preenche a tabela do modal de reavaliação com os dados de carga semanal.
 * @param {Array<object>} dadosCarga - O array de dados de carga gerado por `planoLogic`.
 */
export function renderizarQuadroReavaliacao(dadosCarga) {
    const corpoTabela = DOMElements.tabelaReavaliacaoBody;
    if (!corpoTabela) return;

    let html = '';
    dadosCarga.forEach(dia => {
        const planosDoDiaHTML = dia.planos.length > 0 
            ? dia.planos.map(planoCarga => `
                <span class="plano-carga-tag">
                    <span class="numero-plano-tag">${planoCarga.numero}</span>
                    ${planoCarga.media} pág.
                </span>
            `).join('')
        ...
        html += `
            <tr>
                ...
                <td class="planos-dia-cell">${planosDoDiaHTML}</td>
                <td><span class="total-paginas-dia">${dia.totalPaginas}</span></td>
            </tr>
        `;
    });
