...
export function showCadastroForm(planoParaEditar = null) {
    ...
    DOMElements.planosPausadosSection.style.display = 'none'; // Garante que o painel de pausados também seja ocultado
...
// --- INÍCIO DA NOVA SEÇÃO: Controle do Modal de Agenda e Download ---
...
function renderizarPainelProximasLeituras(planos, totalPlanos) {
    ...
    const todasAsProximasLeituras = planosAtivos.flatMap((plano, planoIndexOriginal) => {
        const planoIndexGlobal = planos.findIndex(p => p.id === plano.id); // Pega o índice do array original
