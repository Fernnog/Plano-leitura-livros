document.addEventListener('DOMContentLoaded', () => {
    ...
    const planosPaginacaoDiv = document.getElementById('planos-paginacao'); // Elemento para paginação


    // Estado inicial
    let planos = carregarPlanosSalvos() || [];
    let planoEditandoIndex = -1;
    let planosPorPagina = 5; // Define quantos planos por página
    let paginaAtual = 1;
    ...
    formPlano.addEventListener('submit', function(event) {
        ...
        if (plano) {
            ...
            salvarPlanos(planos);
            paginaAtual = 1; // Resetar para a primeira página ao salvar um novo plano
            renderizarPlanos();
            formPlano.reset();
            periodoPorDatasDiv.style.display = 'block';
            periodoPorDiasDiv.style.display = 'none';
    ...
    function criarPlanoLeitura(titulo, paginaInicio, paginaFim, dataInicio, dataFim, periodicidade, diasSemana) {
        ...
        let paginaAtualInterno = paginaInicio;

        diasPlano.forEach((dia, index) => {
            const paginasDia = index < resto ? paginasPorDia + 1 : paginasPorDia;
            dia.paginaInicioDia = paginaAtualInterno;
            dia.paginaFimDia = paginaAtualInterno + paginasDia - 1;
            dia.paginas = paginasDia;
            paginaAtualInterno = dia.paginaFimDia + 1;
        });
    ...
    function renderizarPlanos() {
        const startIndex = (paginaAtual - 1) * planosPorPagina;
        const endIndex = startIndex + planosPorPagina;
        const planosPagina = planos.slice(startIndex, endIndex);

        listaPlanos.innerHTML = planosPagina.length === 0 && planos.length > 0 ? '<p>Nenhum plano de leitura nesta página.</p>' : planos.length === 0 ? '<p>Nenhum plano de leitura cadastrado ainda.</p>' : '';

        renderizarPaginacao(); // Renderiza a paginação

        planosPagina.forEach((plano, index) => {
            const planoIndexGeral = planos.indexOf(plano); // Índice do plano no array completo
            ...
            const avisoAtrasoHTML = diasAtrasados > 0 ? `
                <div class="aviso-atraso">
                    ...
                    <button onclick="recalcularPlano(${planoIndexGeral})">Recalcular Plano</button>
                </div>` : '';
            ...
            planoDiv.innerHTML = `
                <div class="plano-header">
                    <h3><span class="plano-numero"> ${planoIndexGeral + 1}. </span>${plano.titulo}</h3>
                    <div>
                        <button onclick="editarPlano(${planoIndexGeral})">Editar</button>
                        <button onclick="excluirPlano(${planoIndexGeral})">Excluir</button>
                    </div>
                </div>
                ${avisoAtrasoHTML}
                ...
                <div class="dias-leitura">${renderizarDiasLeitura(plano.diasPlano, planoIndexGeral)}</div>
            `;
    ...
    function renderizarPaginacao() {
        planosPaginacaoDiv.innerHTML = ''; // Limpa a paginação existente
        if (planos.length <= planosPorPagina) return; // Não precisa paginar se houver poucos planos

        const totalPaginas = Math.ceil(planos.length / planosPorPagina);
        ...
        for (let i = 1; i <= totalPaginas; i++) {
            ...
            link.addEventListener('click', (event) => {
                ...
                paginaAtual = i;
                renderizarPlanos();
                // Remove classe 'active' de todos os links e adiciona ao atual
                document.querySelectorAll('#planos-paginacao a').forEach(a => a.classList.remove('active'));
                link.classList.add('active');
            });
            li.appendChild(link);
            planosPaginacaoDiv.appendChild(li);
        }
    }
    ...
    window.recalcularPlano = function(index) {
        ...
        let paginaAtualInterno = firstNotLidoIndex === 0 ? plano.paginaInicio : plano.diasPlano[firstNotLidoIndex - 1].paginaFimDia + 1;
    ...
    importarPlanosInput.addEventListener('change', function(event) {
        ...
        reader.onload = function(e) {
            try {
                ...
                salvarPlanos(planos);
                paginaAtual = 1; // Resetar para a primeira página ao importar novos planos
                renderizarPlanos();
            } catch (error) {
    ...
    function gerarICS(plano, horarioInicio, horarioFim) {
        ...
        // Data de Fim do Evento (fim do horário no primeiro dia, irrelevante para recorrência, mas necessário)
        ...
        calendarICS += `DESCRIPTION:Plano de leitura do livro "${plano.titulo}". Para mais detalhes e para marcar o progresso da leitura, acesse: <a href="https://fernnog.github.io/Plano-leitura-livros/">Gerenciador de Planos de Leitura</a>\r\n\r\n`;
        ...
        calendarICS += `\r\nAbra o Gerenciador de Planos de Leitura para mais detalhes: https://fernnog.github.io/Plano-leitura-livros/\r\n`;
        calendarICS += `LOCATION:Sua casa ou local de leitura preferido\r\n`;

        // Alarme de 15 minutos antes
        calendarICS += `BEGIN:VALARM\r\n`;
        calendarICS += `TRIGGER:-PT15M\r\n`; // 15 minutos antes
        calendarICS += `ACTION:DISPLAY\r\n`;
        calendarICS += `DESCRIPTION:Lembrete de leitura do livro "${plano.titulo}"\r\n`;
        calendarICS += `END:VALARM\r\n`;
