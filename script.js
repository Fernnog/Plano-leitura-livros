document.addEventListener('DOMContentLoaded', () => {
    ...
    function gerarICS(plano, horarioInicio, horarioFim) {
        ...
        plano.diasPlano.forEach(dia => {
            const dataISO = dia.data.toISOString().slice(0, 10).replace(/-/g, "");
            const inicioEvento = `${dataISO}T${horarioInicio.replace(/:/g, "")}00`;
            const fimEvento = `${dataISO}T${horarioFim.replace(/:/g, "")}00`;

            calendarICS += `BEGIN:VEVENT\r\n`;
            calendarICS += `UID:${dia.data.getTime()}-${plano.id}@gerenciador-planos-leitura\r\n`;
            calendarICS += `DTSTART:${inicioEvento}\r\n`;
            calendarICS += `DTEND:${fimEvento}\r\n`;
            calendarICS += `SUMMARY:Leitura: ${plano.titulo} - Páginas ${dia.paginaInicioDia} a ${dia.paginaFimDia}\r\n`;
            calendarICS += `DESCRIPTION:Plano de leitura do livro "${plano.titulo}". Leia as páginas ${dia.paginaInicioDia} a ${dia.paginaFimDia}.\nAbra o Gerenciador de Planos de Leitura para mais detalhes.\r\n`;
