document.addEventListener('DOMContentLoaded', () => {
    ...
    const paginacaoPlanosDiv = document.getElementById('paginacao-planos');
    ...
    const planosPorPagina = 3; // Define quantos planos por página
    ...
    function renderizarPlanos() {
        if (planos.length === 0) {
            ...
            paginacaoPlanosDiv.innerHTML = ''; // Limpa a paginação se não há planos
            return;
        }

        const numPaginas = Math.ceil(planos.length / planosPorPagina);
        renderizarPaginacao(numPaginas);

        const paginaAtual = parseInt(paginacaoPlanosDiv.dataset.paginaAtual) || 1;
        const inicioIndex = (paginaAtual - 1) * planosPorPagina;
        const fimIndex = inicioIndex + planosPorPagina;
        const planosPagina = planos.slice(inicioIndex, fimIndex);

        listaPlanos.innerHTML = ''; // Limpa a lista antes de renderizar a página atual
        planosPagina.forEach((plano, indexNaPagina) => {
            renderizarPlanoHTML(plano, inicioIndex + indexNaPagina);
        });
    }

    function renderizarPaginacao(numPaginas) {
        paginacaoPlanosDiv.innerHTML = ''; // Limpa os links de paginação existentes
        const paginaAtual = parseInt(paginacaoPlanosDiv.dataset.paginaAtual) || 1;

        if (numPaginas <= 1) return; // Não mostra paginação se só tem uma página ou menos
        ...
        for (let i = 1; i <= numPaginas; i++) {
            ...
            if (i === paginaAtual) {
                linkPagina.classList.add('pagina-atual'); // Adicione uma classe para a página atual, se precisar estilizar
            }
            linkPagina.addEventListener('click', (event) => {
                event.preventDefault();
                paginacaoPlanosDiv.dataset.paginaAtual = i;
                renderizarPlanos();
            });
            paginacaoPlanosDiv.appendChild(linkPagina);
        }
    }

    function renderizarPlanoHTML(plano, index) {
    ...
    function gerarICS(plano, horarioInicio, horarioFim) {
        ...
        calendarICS += `DESCRIPTION:Plano de leitura do livro "${plano.titulo}".\r\n`;
        calendarICS += `Para mais detalhes ou para gerenciar seus planos de leitura, acesse: ${'https://fernnog.github.io/Plano-leitura-livros/'}\r\n\r\n`;
        ...
        calendarICS += `DESCRIPTION:Lembrete de leitura: ${plano.titulo} - Páginas do dia.\r\n`;
