...
function setupEventHandlers() {
    ...
    // --- NOVO: Modal de Reavaliação de Carga Semanal ---
    ...
    DOMElements.reavaliacaoModal.addEventListener('click', (event) => {
        // Fecha o modal se o clique for no overlay (fora do conteúdo)
        if (event.target === DOMElements.reavaliacaoModal) {
...
// --- NOVO: Handler para o Modal de Reavaliação ---
function handleReavaliarCarga() {
    const planosAtuais = state.getPlanos();
    // A função de lógica precisa saber o total de planos para a numeração visual (ex: #5, #4...)
    const totalPlanos = planosAtuais.length; 
    
    // 1. Chama a lógica de negócio para analisar os dados
    const dadosCarga = planoLogic.analisarCargaSemanal(planosAtuais, totalPlanos);
    
    // 2. Chama a UI para renderizar a tabela com os dados
